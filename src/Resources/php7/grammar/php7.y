...
non_empty_type_argument_list:
      type                                                  { init($1); }
    | non_empty_type_argument_list ',' type                 { push($1, $3); }
;
...
non_empty_type_parameter_list:
      T_STRING                                              { init($1); }
    | non_empty_type_parameter_list ',' T_STRING            { push($1, $3); }
;
...
function_declaration_statement:
    T_FUNCTION optional_ref identifier type_parameter_list '(' parameter_list ')' optional_return_type block_or_error
        { $$ = ExStmt\Function_[$3, ['typeParams' => $4, 'byRef' => $2, 'params' => $6, 'returnType' => $8, 'stmts' => $9]]; }
;

class_declaration_statement:
      class_entry_type identifier type_parameter_list extends_from implements_list '{' class_statement_list '}'
          { $$ = ExStmt\Class_[$this->handleGenericType($2, $3), ['type' => $1, 'extends' => $4, 'implements' => $5, 'stmts' => $7]];
            $this->checkClass($$, #2); }
    | T_INTERFACE identifier type_parameter_list interface_extends_list '{' class_statement_list '}'
          { $$ = ExStmt\Interface_[$this->handleGenericType($2, $3), ['extends' => $4, 'stmts' => $6]];
            $this->checkInterface($$, #2); }
    | T_TRAIT identifier type_parameter_list '{' class_statement_list '}'
          { $$ = ExStmt\Trait_[$this->handleGenericType($2, $3), ['stmts' => $5]]; }
;
...
type:
      name type_argument_list                               { $$ = $this->handleArgumentedType($this->handleBuiltinTypes($1), $2); }
...
class_statement:
    ...
    | method_modifiers T_FUNCTION optional_ref identifier_ex type_parameter_list '(' parameter_list ')' optional_return_type method_body
          { $$ = ExStmt\ClassMethod[$4, ['type' => $1, 'typeParams' => $5, 'byRef' => $3, 'params' => $7, 'returnType' => $9, 'stmts' => $10]];
...
function_call:
    ...
    | class_name_or_var T_PAAMAYIM_NEKUDOTAYIM member_name type_argument_list argument_list
          { $$ = Expr\StaticCall[$1, $3, $5]; }
;

class_name:
      T_STATIC                                              { $$ = Name[$1]; }
    | name type_argument_list                               { $$ = $this->handleArgumentedType($1, $2); }
;
